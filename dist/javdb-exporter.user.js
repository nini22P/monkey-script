...
function exportVideosInfo() {
  ...
  const dateTime = (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").split(".")[0];
  let fileName = "";
...
if (url.includes("/watched_videos") || url.includes("/want_watch_videos") || url.includes("/list_detail") || url.includes("/lists")) {
  createExportButton();
  if (allowExport) {
    startExport();
  }
